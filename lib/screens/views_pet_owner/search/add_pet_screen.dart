...
class AddPetsScreen extends StatefulWidget {
  ...
  @override
  State<AddPetsScreen> createState() => _AddPetScreenState();
}

class _AddPetScreenState extends State<AddPetsScreen> {
  ...
  void initState() {
    ...
    _petService = PetService(); // Inicializar el servicio
    _requestPermissions();
  }
  ...
  // Función para validar el formulario y guardar la mascota - MODIFICADA
  void _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      // Convertir Species enum basado en el tipo ingresado
      Species petSpecies;
      String petType = _typeController.text.toLowerCase();
      if (petType.contains('perro') || petType.contains('dog')) {
        petSpecies = Species.DOG;
      } else if (petType.contains('gato') || petType.contains('cat')) {
        petSpecies = Species.CAT;
      } else {
        petSpecies = 'OTHER' as Species;
      }

      // Crear objeto de mascota usando el modelo Pet correcto
      final newPet = Pet(
        ...
        imgUrl: _petImages.first.path, // Usar la primera imagen como principal
      ...
      try {
        // Guardar mascota usando PetService directamente
        if (_petService != null) {
          //bool success = await _petService!.createPet(newPet)
