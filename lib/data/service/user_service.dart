...
import '../models/user/owners_with_pets.dart';
  ...
  class UserService {
    ...
    /// Obtener detalles de todos los dueños (solo para CAREGIVER)
    /// Endpoint: GET /users/owners/details
    Future<List<OwnerDetailResponse>?> getAllOwnersDetails() async {
      try {
        final url = Uri.parse('$_baseUrl/users/owners/details');
        final response = await http.get(url, headers: _headers);

        print('Get all owners details response: ${response.statusCode}');
        print('Get all owners details body: ${response.body}');

        if (response.statusCode == 200) {
          final jsonData = json.decode(response.body);
          if (jsonData is List) {
            return jsonData
                .map((ownerJson) => OwnerDetailResponse.fromJson(ownerJson))
                .toList();
          } else {
            throw Exception('Expected a list of owners but got: ${jsonData.runtimeType}');
          }
        } else if (response.statusCode == 404) {
          print('No se encontraron dueños');
          return [];
        } else if (response.statusCode == 403) {
          print('No tiene permisos para obtener detalles de dueños (requiere rol CAREGIVER)');
          return null;
        } else {
          throw Exception('Failed to load owners details: ${response.statusCode} - ${response.body}');
        }
      } catch (e) {
        print('Error al obtener detalles de dueños: $e');
        return null;
      }
    }
